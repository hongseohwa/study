...
select sum(sal), count(*), avg(sal), max(sal), min(sal)
from emp
group by deptno
order by sum(sal) desc;
...
/*
   ...
   - View를 만들면 데이텁이스에 질의 시 실제 테이블에 접근하여 데이터를 불러오게 됨.
   ...
   - View 를 사용하는 이유
     1) 보안관리를 위해 사용함(중요함) ==> 보안 등급에 맞추어 컬럼의 범위를 정해서 조회가 가능하도록 할 수 있음.
     2) 사용자의 편의성을 제공함.
     형식) 
        create view 뷰 이름
        as
...
-- 인사부 View 컬럼에 sal(급여), comm(보너스) 컬럼은 제외
create view emp_insa
as
select empno, ename, job, mgr, hiredate, deptno
from emp;

-- 영업부 View 컬럼에 sal(급여) 컬럼은 제외.
...
-- View 를 읽기 전용으로 만들면 데이터 추가가 안 됨.
-- 읽기 전용으로 만드는 방법 ==> View 만들 때 쿼리문 맨 마지막에 with read only 문구 추가;
create view emp_view1
as
select * from emp
with read only;

insert into emp_view1 values(9001, '홍서화', 'SALESMAN', 7698, sysdate, 1500, 200, 30);

-- create or replace view : 같은 이름의 view가 있는 경우에는 기존의 view는 삭제하고 새로운 view를 만들라는 의미.
...
insert into emp_account values(9001, '홍서화', 'SALESMAN', 7698, sysdate, 1500, 200, 30);

-- 2) 사용자의 편의성을 제공함.
create or replace view emp_salary
as
select empno, ename, (sal * 12 + nv12(comm, comm, 0)) 연봉
from emp
with read only;

-- [문제1] 부서별로 부서별 급여 합계, 부서별 급여 평균을 구한 view를 만들어 화면에 보여주세요.
-- 주의사항) view를 만들 때 그룹함수 사용시에는 반드시 별칭을 설정해 주어야 함.
create or replace view emp_deptno_sal
as
...
-- [문제2] emp 테이블을 이용하여 emp_dept20 이라는 view를 만들어 주세요. 
-- 단, 부서번호가 20번 부서에 속한 사원들의 사번, 이름, 담당업무, 관리자, 부서번호만 화면에 보여주시기 바랍니다.
...
-- View 를 만들 때 컬럼만 만들고 싶은 경우 ==> 조건을 줄 때 말이 안되는 조건을 작성하면 됨.
...
/*
   ...
   - 명령어 여러 개의 집합이 정상적으로 처리가 되면 종료하고, 여러 개의 명령어 중에서 하나의 명령이라도 잘못이 되면 전체를 취소하는 것을 말함. - 중요함
   
   - 트랜잭션 사용 이유 : 데이터의 일관성을 유지하면서 데이터의 안정성을 보장하기 위해 사용함.
   - 트랜잭션 사용 시 트랜잭션을 제어하기 위한 명령어.
   1) commit : 모든 작업을 정상적으로 처리하겠다고 확정하는 명령어. 트랜잭션(insert, update, delete) 작업의 내용을 실제 DB에 반영.
               이전에 있던 데이터에 update 현상이 발생을 함. 모든 사용자가 변경된 데이터의 결과를 볼 수 있음.
   2) rollback : 작업 중에 문제가 발생했을 때 트랜잭션 처리 과정에서 발생한 변경 사항을 취소하여 이전 상태로 되돌리는 명령어.
                 트랜잭션(insert, update, delete) 작업의 내용을 취소함. 이전에 commit 한 곳까지만 복구가 됨.
*/
...
-- 형식) delete from 테이블이름 where primary key 컬럼명 = 내용(데이터)
...
-- 4. 이 때 만일 20번 부서에 대해서만 삭제를 하고 싶었는데 잘못해서 전체가 삭제가 된 경우, 다시 복원을 할 수 있음.
rollback;
...
/*
   savepoint
   - 트랜잭션을 작게 분할하는 것을 말함.
   - 사용자가 트랜잭션 중간 단계에서 포인트를 지정하여 트랜잭션 내의 특정 savepoint 까지 rollback 할 수 있게 하는 것을 말함.
*/
...
-- 8. 부서번호가 20번인 부서를 삭제하기 바로 전으로 되돌아가고 싶어함.
rollback sp1;

-- student 테이블의 특정 레코드를 수정하는 방법
-- 형식) update 테이블이름 set
--             수정할 컬럼 이름1 = 수정될 컬럼 내용, 
--             수정할 컬럼 이름2 = 수정될 컬럼 내용, 
--             where primary key 컬럼명 = 값;
-- 주의사항) 만약 where 조건절이 누락이 되면 테이블 전체 데이터가 수정이 이루어짐.

update student set
       major = '해양학과', addr = '전라남도 신안군'
       where hakbun = '2023_002';

update student set
       major = '역사학과', addr = '수원 화성'
       where hakbun = '2023_001';
       
update student set
       major = '컴퓨터공학과', addr = '부산'
       where hakbun = '20193864';
       
/*
   PL/SQL(Procedual Language / SQL)
   - SQL 만으로는 구현이 어렵거나 구현 불가능한 작업을 수행하기 위해서 오라클에서 제공하는 프로그래밍 언어를 말함.
   - 일반적으로 프로그래밍 언어적인 요소들을 다 가지고 있으며, 데이터베이스 업무를 처리하기 위한 최적화된 언어.
   - 변수, 조건 처리,  반복처리 등 다양한 기능을 사용할 수 있음.
   ...
   - 기본 구조
    ...
    - 위 기본 구조 중에서 선언부와 에외처리부는 생략이 가능하지만 실행부는 반드시 존재해야 함.
...
-- 오라클에서는 래퍼런스(reference) 변수를 제공해 줌.
...
begin
    ...
    dbms_output.put_line(emp_row.empno || ' ' || emp_row.ename || ' ' || emp_row.job || ' ' || emp_row.mgr 
    || ' ' || emp_row.hiredate || ' ' || emp_row.sal || ' ' || emp_row.comm || ' ' || emp_row.deptno);
end;
/
...
-- 2) if ~ then ~ else
-- [문제] 변수에 들어 있는 값이 홀수인지, 짝수인지 판별하여 화면에 출력해 보세요.
declare
    e_number number(3) := 20;
begin
    if mod(e_number, 2) = 0 then
        dbms_output.put_line(e_number || '는 짝수 입니다.');
    else
        dbms_output.put_line(e_number || '는 홀수 입니다.');
    end if;
end;
/
...
/*
    ...
    - 반복문의 반복적인 수행을 종료시키는 명령어.
      ...
      2) exit ~ when - 반복 종료를 위한 조건식을 지정하고 만족하면 반복을 종료시키는 명령어.
      3) continue - 수행 중인 반복의 현재 주기를 건너 뛰는 명령어.
      4) continue ~ when - 특정 조건식을 지정하고 조건식을 만족하면 반복 주기를 건너 뛰는 명령어.
*/
...
declare
    ...
    e_sum number(5) := 0;
begin
    dbms_output.put_line('** while 문을 이용한 1 ~ 100 까지의 합 ***');
...
declare
    e_sum number(5) := 0;
begin
    dbms_output.put_line('** for 문을 이용한 1 ~ 100 까지의 합 ***');
