/*
   시퀀스(sequence)
   ==> 순서를 부여할 때 사용하는 문법, 연속적인 번호를 만들어 주는 기능을 함.
   ...
   형식) create sequence 시퀀스이름
        ...
        1) cache : 시퀀스를 빠르게 제공하기 위해 미리 캐쉬 메모리에 시퀀스를 넣어 두고 준비하고 있다가 시퀀스 작업이 필요할 때 사용을 함.
...
insert into memo
values(memo_seq.nextval, '메모2', '이순신', '장군님 메모', sysdate);
insert into memo
values(memo_seq.nextval, '메모3', '유관순', '유관순 열사 메모', sysdate);

commit;

/*
   제약 조건
   - 테이블에 부적합한 자료가 입력되는 것을 방지하기 위해서 테이블을 생성할 때 각 컬럼에 대해서 정의하는 여러 가지 규칙을 정한 것을 말함.
...
/*
   1) not null 제약 조건
      ...
      - 특정 열에 데이터의 중복 여부와는 상관 없이 null 값을 허용하지 않는 제약 조건.
*/
...
/*
   4) foreign key 제약 조건
      ...
      - 자식 테이블의 컬럼의 값(데이터)이 부모 테이블에 없는 경우 무결성의 규칙이 깨져 버림.
      - 외래 키가 존재하기 위해서는 우선적으로 부모 테이블이 먼저 만들어져야 함.
      - 옵션
        * on delete cascade 
        ==> 부모 테이블의 데이터 삭제 시 해당 데이터를 참조하고 있는 자식 테이블의 데이터까지 모두 삭제하는 옵션
        * on delete set null
        ==> 부모 테이블의 데이터 삭제 시 해당 데이터를 참조하고 있는 자식 테이블의 컬럼 값을 null로 변경하는 옵션
*/
...
-- dept number(2) references dept(deptno) -- 컬럼상 외래키 제약 조건
dept number(2),
constraint dept_fk foreign key(dept) 
references dept(deptno) -- 테이블상 외래키 제약 조건
);
...
/*
   ...
   - 두 개의 테이블로 나뉘어진 경우에는 데이터의 중복은 발생하지 않지만 원하는 정보를 얻으려면 여러 번 질의를 해야 하는 불편함이 발생됨.
   - 특정 사원이 소속된 부서의 부서명을 알고 싶은 경우에는 emp 테이블과 dept 테이블을 넘나드는 작업이 생기므로 상당히 불편함.
...
-- [문제2] emp 테이블에서 'NEW YORK' 에 근무하는 사원의 이름과 급여, 부서번호를 화면에 보여주세요.
select ename, sal, d.deptno
from emp e join dept d
on e.deptno = d.deptno
where loc = 'NEW YORK';

-- [문제3] emp 테이블에서 'ACCOUNTING' 부서 소속 사원의 이름과 담당업무, 입사일 그리고 부서번호, 부서명을 화면에 보여주세요.
select ename, job, hiredate, d.deptno, dname
from emp e join dept d
on e.deptno = d.deptno
where dname = 'ACCOUNTING';

-- [문제4] emp 테이블에서 담당업무가 'SALESMAN' 인 사원의 이름과, 담당업무, 부서번호, 부서명, 근무위치를 화면에 보여주세요.
...
/*
   3) Self Join
      - 하나의 테이블 내에서 조인을 해야 자료를 얻을 수 있는데 말 그대로 자기 자신 테이블과 조인을 맺는 것을 말함.
      - from 절 다음에 테이블 이름을 나란히 두 번 기술을 할 수 없음. 따라서 같은 테이블이 하나 더 존재하는 것처럼 사용할 수 있도록 테이블에 별칭을 붙여서 사용해야 함.
*/
...
-- emp 테이블에서 매니저가 'KING' 인 사원들의 이름과 담당업무를 화면에 보여주세요.
...
/*
   4) Outer Join
      - 2개 이상의 테이블이 조인이 될 때 어느 한 쪽 테이블에는 해당되는 데이터가 다른쪽 테이블에는 데이터가 존재하지 않는 경우
