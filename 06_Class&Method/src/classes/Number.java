 ...
 * - 예를 든다면 캡슐 약을 생각하면 쉬움. 캡슐에 든 약은 약이 어떤 색인지, 어떤 성분인지 보이지 않으며, 외부의 접근으로부터 안전함.
 ...
 * - TV는 내부를 캡슐화하고 on/off 버튼, 밝기 조절 버튼, 채널 버튼, 음량 버튼, 리모컨 과 통신하는 부분은 노출을 시킴. 
...
public class Number {
	 ...
	 * - 주로 멤버변수와 메서드 또는 생성자의 매개변수 이름이 동일할 때 객체의 멤버임을 명확하게 하기 위해서 사용되는 키워드.
	 * - 매개변수(지역변수)의 이름과 멤버변수(전역변수)의 이름이 같을 때 멤버변수(전역변수) 앞에 this 라는 키워드를 붙여서 구분할 때 사용을 함.
	 * - 메서드 내에서 지역변수와 전역변수의 이름이 같으면 지역변수가 우선순위가 높음. 
	 */
