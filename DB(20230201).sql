...
--    5) months_between('마지막날짜', 현재날짜) : 두 날짜 사이의 개월 수를 출력해 주는 함수.
...
--    4) substr('문자열', x, y) : 문자열을 x부터 y의 길이만큼 추출해 주는 함수.
select substr('ABCDEFG', 3, 2) from dual;

--    4) substr('문자열', x, y) : x 값이 음수인 경우에는 오른쪽부터 시작이 된다.
select substr('ABCDEFG', -3, 2) from dual;

--    5) 자릿수를 늘려주는 함수.
--       5-1) lpad('문자열', '전체자릿수', '늘어난 자릿수에 들어갈 문자열') : 왼쪽 자릿수를 늘려 주는 함수.
select lpad('ABCDEFG', 15, '*') from dual;

--       5-2) rpad('문자열', '전체자릿수', '늘어난 자릿수에 들어갈 문자열') : 오른쪽 자릿수를 늘려 주는 함수.
...
-- 결과) '홍길동 회원의 직업은 학생입니다.' 단, concat() 함수를 이용하세요.
...
select substr('000121-1111111', 1, 6) from dual;

-- [문제6] emp 테이블에서 담당업무에 'A' 라는 글자를 '$'로 바꾸어 화면에 보여주세요.
select replace(job, 'A', '$') from emp;

-- [문제7] member10 테이블에서 직업이 '학생' 인 정보를 '대학생'으로 바꾸어 화면에 보여주세요.
...
select round(2.3424557, 4) from dual;
select round(1236.5678, -2) from dual;
...
/*
   ...
   - 여러 번 쿼리를 실행해서 얻을 수 있는 결과를 하나의 중첩된 쿼리 문장으로 결과를 얻을 수 있게 해 줌.
   
   ※ 주의사항
      - 서브쿼리는 괄호로 묶어서 사용하는 것이 관례임.
      - 서브쿼리 안에서는 order by 절은 사용할 수 없음.
      
    - 사용방법 : 우선은 안쪽에 있는 쿼리문을 실행 후, 그 결과값을 가지고 바깥쪽 쿼리문을 실행함.
*/

-- emp 테이블에서 이름이 'SCOTT' 인 사원의 급여보다 더 많은 급여를 받는 사원의 사번, 이름, 담당업무, 급여를 화면에 보여주세요.
...
-- [문제2] emp 테이블에서 사번이 7521인 사원과 담당업무가 같고, 급여가 사번이 7934인 사원보다 더 많이 받는 
-- 사원의 사번, 이름, 담당업무, 급여를 화면에 보여주세요.
select empno, ename, job, sal
from emp
where job = (select job from emp where empno = 7521) and sal > (select sal from emp where empno = 7934);

-- [문제3] emp 테이블에서 담당업무가 'MANAGER' 인 사원의 최소급여보다 적으면서, 담당업무가 'CLERK'은 아닌
-- 사원의 사번, 이름, 담당업무, 급여를 화면에 보여주세요.
select empno, ename, job, sal
from emp
where sal < (select min(sal) from emp where job = 'MANAGER') and job != 'CLERK';
...
-- [문제5] member10 테이블에 있는 고객의 정보 중 마일리지가 가장 높은 고객의 모든 정보를 화면에 보여주세요.
select *
from member10
...
-- [문제8] emp 테이블에서 'BLAKE'와 같은 부서에 있는 사원들의 이름과 입사일자, 부서번호를 화면에 보여주되, 'BLAKE' 는 제외하고 화면에 보여주세요.
...
-- [문제13] emp 테이블에서 자신의 급여가 평균급여보다 많고, 이름에 'S' 자가 들어가는 사원과 동일한 부서에서 근무하는 
-- 모든 사원의 사번, 이름, 급여, 부서번호를 화면에 보여주세요.
...
-- [문제16] products 테이블에 있는 판매 가격에서 평균 가격 이상의 상품 목록을 구하되, 평균을 구할 때 가격이 가장 큰 금액인 상품을
-- 제외하고 평균을 구하여 화면에 보여주세요.
select *
from products
where output_price > (select avg(output_price) from products where output_price <> (select max(output_price) from products));

-- [문제17] products 테이블에서 상품명의 이름에 '에어컨' 이라는 단어가 포함된 카테고리에 속하는 상품목록을 화면에 보여주세요.
select products_name
from products
where category_fk = (select distinct category_fk from products where products_name like '%에어컨%');

-- [문제18] member10 테이블에 있는 고객 정보 중 마일리지가 가장 높은 금액을 가지는 고객에게 보너스 마일리지 5000점을 더 주어 
-- 고객명, 마일리지, 마일리지+5000 점을 화면에 보여주세요.
