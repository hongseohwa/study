 ...
 *   * getParameterValues(String name) => 같은 name에 대하여 여러 개의 값을 얻을 때 사용함. 반환형은 String[] 타입임.
 *                                   
 ...
 * 서블릿 생명 주기(Life Cycle)
	 ...
	 * 
	 * - init() : 서블릿 요청 시 맨 처음 한 번만 호출됨. 서블릿 생성 시 초기화 작업을 주로 진행을 함.
	 * - doGet() / doPost() : 서블릿 요청 시 매번 호출됨. 실제로 클라이언트가 요청하는 작업을 수행함.
	 * - destory() : 서블릿이 기능을 수행하고 메모리에서 소멸될 때 호출이 됨. 서블릿의 마무리 작업을 주로 수행을 함.
	 ...
	 *                         웹 브라우저는 기본적으로 HTML만 인식하므로 서블릿에서 전송하는 대부분의 데이터는 MIME-TYPE을 "text/html"로 설정을 함.
...
public class JoinServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1단계 : Ex05.jsp 폼 페이지에서 넘어온 정보들을 받자.
