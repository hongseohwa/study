...
public class EMP extends JFrame{
	...
	public EMP() { // 기본 생성자
		...
		String[] header = {"사 번", "이 름", "담당업무", "관리자No.", "급 여", "보너스", "부서번호", "입사일"};
		
		model = new DefaultTableModel(header, 0); // 데이터 추가, 수정을 집어넣음
		table = new JTable(model); // JTable에서 보여줌, 단순히 보여주기 위한 용도
		...
		// 5. 이벤트 처리
		// 사원목록(btn1) 버튼을 클릭 시 EMP 테이블에 있는 전체 사원의 목록을 JTable에 띄워주면 됨.
	...
	void select() {
		...
		try {
			...
			while(rs.next()) {
				...
				Object[] record = {empno, ename, job, mgr, sal, comm, deptno, hiredate};
				model.addRow(record);
			}
