...
-- 테이블에 컬럼을 수정하는 방법(컬럼의 전체적인 내용 수정)
...
/*
   ...
   3. DCL(Data Control Language) : 데이터 제어 언어
      - GRANT : 특정 데이터 베이스 사용자에게 작업의 특정 수행 권한을 부여하는 명령어.
...
-- 형식1) insert into 테이블이름 values(첫번째 컬럼 데이터.. 마지막 컬럼 데이터);
-- values는 null값이 허용된다 하더라도 모든 컬럼에 데이터를 넣어줘야 함.
insert into student values('2023_001', '홍길동', '경제학과', '010-1111-1234', '서울시 중구 남대문로', sysdate);

-- 형식2) insert into 테이블이름(not null 제약조건 컬럼들 명시) values(제약 조건 컬럼 수에 해당하는 데이터 기재)
insert into student(hakbun, name1, phone, regdate) values ('2023_002', '이순신', '010-2222-2345', sysdate);

insert into student values('20193864', '홍서화', '항공서비스전공', '010-8527-0867', '경기도 화성시', sysdate);
insert into student(hakbun, name1, phone, regdate) values('20191111', '이나림', '010-2209-7334', sysdate);
insert into student(hakbun, name1, phone, regdate) values('20192222', '홍대희', '010-8831-0388', sysdate);
...
-- [문제3] emp 테이블에서 사번, 이름, 급여, 보너스, 급여 + 보너스 를 화면에 보여주세요.
...
--            보통 별칭은 "" 안에 작성을 함. as라는 키워드를 생략해도 됨.
select empno, ename, sal, comm, sal + nvl2(comm, comm, 0) as "SAL + COMM" from emp;
...
-- emp 테이블에서 각 사원의 관리자(mgr)를 중복이 되지 않게 화면에 보여주세요.
...
-- || : 연결 연산자.
select ename || '사원의 연봉은 ' || sal * 12 || '입니다.' "연봉" from emp;

-- [문제] member10 테이블의 내용을 아래와 같이 출력해 보자.
-- 예) 홍길동 회원의 직업은 학생이고, 마일리지는 0 입니다.
select memname || ' 회원의 직업은 ' || job || '이고, 마일리지는 ' || mileage || ' 입니다.' "정보" from member10;
...
/*
   ★★★★★★★★★★★★★★★★★★★
   where 조건절
   - 모든 데이터를 가져 오는 것이 아니라 사용자가 원하는 데이터만 조회할 경우에 사용됨.
...
-- [문제2] emp 테이블에서 1982년 1월 1일 이후에 입사한 사원의 사번, 이름, 담당업무, 급여, 입사일자를 화면에 보여주세요.
...
-- [문제5] emp 테이블에서 보너스가 300이거나 500이거나 1400인 사원의 모든 정보를 화면에 보여주세요.
select * from emp
...
select products_name, input_price, output_price from products
